...
pub struct SpecializationPlugin<M>(PhantomData<M>)
where
    M: SpecializedMaterial;

impl<M> Default for SpecializationPlugin<M>
where
    M: SpecializedMaterial,
{
    fn default() -> Self {
...
impl<M> Plugin for SpecializationPlugin<M>
where
    M: SpecializedMaterial,
{
    fn build(&self, app: &mut App) {
        app.add_plugins(ExtractResourcePlugin::<ViewKeyCache<M::ViewKey>>::default())
            .init_resource::<ViewKeyCache<M::ViewKey>>()
            .add_systems(First, clear_needs_specialization::<M>)
            .add_systems(
                PostUpdate,
                (
...
pub trait CheckNeedsSpecialization: Send + Sync + 'static {
...
pub fn check_entity_needs_specialization<M>(
    mut commands: Commands,
    query: Query<(Entity, M::ViewQueryData), M::ViewQueryFilter>,
) where
    M: SpecializedMaterial,
{
...
pub fn check_views_need_specialization<M>(
    mut commands: Commands,
    mut view_key_cache: ResMut<ViewKeyCache<M::ViewKey>>,
    views: Query<(
        Entity,
        &VisibleEntities,
        <<M as SpecializedMaterial>::ViewKey as SpecializedViewKey>::ViewQueryData,
    )>,
) where
    M: SpecializedMaterial,
{
    for (view_entity, visible_entities, view_query) in &views {
        let view_key = M::ViewKey::get_view_key(view_query);
            ...
            if *current_key != view_key {
                *current_key = view_key;
                let batch = visible_entities
                    .iter(TypeId::of::<M::VisibilityClass>())
                    .copied()
                    .map(|entity| (entity, NeedsSpecialization))
                    .collect::<Vec<_>>();
